...
// gm special handling of dla entry exit
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName: EnteredGeofence object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification * _Nonnull note) {
        CLCircularRegion *region =(CLCircularRegion *)note.userInfo[@"region"] ;
        CLLocationCoordinate2D center =region.center;
        CLLocationDistance radius =region.radius;

        
        GMHttpRestApi *api = [[GMHttpRestApi alloc ]init];
        [api doCallApi:center.latitude andLon:center.longitude];
        NSLog(@"Entry location lat:%f longitude:%f, radius:%f",center.latitude,center.longitude,radius);
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName: ExitedGeofence object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification * _Nonnull note) {
       ...
       // NSLog(@"EXIT location lat:%f longitude:%f, radius:%f",center.latitude,center.longitude,radius);
    }];
...
//        [UrlInboxActionPlugin registerPlugin];
